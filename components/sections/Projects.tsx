          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {displayedProjects.map((project, index) => {
              ...
              return (
                <Card 
                  key={project.id} 
                  className={`cursor-pointer transition-all duration-700 ease-out flex flex-col h-full ${
                    isFeatured 
                      ? 'animate-slide-up' 
                      : showAll 
                ...
                >
                  {/* Project Image */}
                  <div className="relative h-48 rounded-xl overflow-hidden mb-6 bg-primary-surface">
                    {project.images && project.images.length > 0 ? (
                      ...
                      <div className="flex items-center justify-center h-full bg-primary-surface">
                        {/* Clean empty state - no emoji or text */}
                      </div>
                    )}
                    {project.featured && (
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-8">
                {modalData.images?.map((image, idx) => (
                  <div key={idx} className="relative h-64 rounded-xl overflow-hidden bg-primary-surface">
                    <Image
                      src={image}
                      alt={`${modalData.title} screenshot ${idx + 1}`}
                      fill
